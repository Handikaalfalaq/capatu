...
**Catatan kepatuhan**: repository dan seluruh dokumentasi/README/commit **tidak boleh mengandung kata "Mandiri"**, sesuai instruksi soal assessment.
...
Lihat detail lengkap di [`DATA-MODEL.md`](./DATA-MODEL.md) — termasuk desain `shoe_type` dan `shoe_feature` sebagai tabel referensi extensible (jenis sepatu & fitur khusus seperti plat karbon/anti air bisa ditambah tanpa deploy ulang), serta detail atribut sepatu (warna, ukuran, berat, harga beli, dll).
